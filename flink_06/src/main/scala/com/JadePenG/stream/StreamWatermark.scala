...
object StreamWatermark {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val orderDataStream: DataStream[Order] = env.addSource(new RichSourceFunction[Order] {
      ...
      override def run(ctx: SourceFunction.SourceContext[Order]): Unit = {
        while (isRunning) {
          ...
          // - 时间戳为当前系统时间
          // - 每隔1秒生成一个订单
          val order = Order(UUID.randomUUID().toString, Random.nextInt(3), Random.nextInt(101), new
    ...
    // 9. 打印结果数据
    // 10. 启动执行流处理
    val resultDStream = watermarkDataStream.keyBy(_.userId)
