...
/**
  ...
  * httpReferrer：refer
  */
//保存结构化数据 (初始化的时候需要传递一堆参数，这些参数实际都是预处理的结果数据)
case class ProcessedData(sourceData: String, requestMethod: String, request: String,
  ...
  implicit class StringUtils(s: String) {
    //传递一个参数，如果这个参数是空值，那就返回NULL，否则原样返回
    //#CS#NULL#CS#
  ...
  def toKafkaString(fieldSeparator: String = "#CS#"): String = {
    ...
    val queryRequestDataStr: String = queryRequestData match {
      case Some(value) =>
        try {
    ...
    val bookRequestDataStr: String = bookRequestData match {
      case Some(value) =>
        try {
          mapper.writeValueAsString(value)
        } catch {
