...
object BlackListToRedis {
  ...
  def dataToRedis(sqlContext:SQLContext,paths:Array[String],jedis: JedisCluster): Unit = {
    ...
    //查询到的数据是最后一次写入到redis的数据明细（爬虫列表）
    val distincted: DataFrame = sqlContext.sql("select a.keyExpTime, a.key, a.value from blacklist a join groupedlist b on a.keyExpTime=b.keyExpTime and a.key=b.key")
    distincted.show()
